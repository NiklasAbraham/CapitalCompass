"""OAM holdings enrichment module.
...
class OAMEnrichment:
    ...
    def enrich_holdings(self, silver_df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Step 1: Ensure ISIN identifiers are present
        gold_df = self._ensure_isin(gold_df)
    ...
    def _ensure_isin(self, df: pd.DataFrame) -> pd.DataFrame:
        """Ensure ISIN identifiers are present.
        
        Args:
            df: DataFrame with holdings
            
        Returns:
            DataFrame with ISINs
        """
        # Create a unified identifier column
        if 'isin' in df.columns:
            df['instrument_isin'] = df['isin']
        else:
            df['instrument_isin'] = None
    ...
    def _compute_weights(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df['market_value_eur'] = market_values
    ...
    def _normalize_classifications(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if 'category_raw' in df.columns:
            ...
            # Map UCITS categories to standard asset classes
            asset_class_map = {
                'Equity': 'Equity',
