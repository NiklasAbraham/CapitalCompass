...
class NPORTQualityAssurance:
    ...
    def validate_holdings(
                ...
                top10_holdings.append(
                    {
                        "name": str(
                            row.get(
                                "instrument_name",
                                row.get("instrument_name_raw", "Unknown"),
                            )
                        ),
        ...
        # Write QA report
        self._write_report(result, fund_name)

        return result

    def _write_report(self, result: QAResult, fund_name: Optional[str] = None) -> None:
        """Write QA report to JSON file.

        Args:
            result: QA result
            fund_name: Clean fund name for directory
        """
        if not fund_name:
            # Fallback to fund_id if name not provided
            fund_name = result.fund_id.replace("=", "_").replace("/", "_")

        report_dir = self.funds_dir / fund_name / result.as_of
