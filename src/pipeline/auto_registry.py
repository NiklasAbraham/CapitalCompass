...
for US-registered funds using N-PORT filings.
"""
...
class AutoRegistry:
    ...
    def ensure_fund_registered(
        ...
        # If entry exists, check if it needs updates (e.g., missing tickers)
        if fund_id:
        ...
        if isin:
            for fund_id, entry in funds.items():
                if isinstance(entry, dict):
                    if (
                        entry.get("share_class_isin") == isin
                        or entry.get("isin") == isin
                    ):
                        return fund_id
    ...
    def _discover_fund_entry(
            ...
            try:
                ...
                for fund_id, fund_entry in funds.items():
                    if isinstance(fund_entry, dict):
                        if (
                            fund_entry.get("share_class_isin") == isin
                            or fund_entry.get("isin") == isin
                        ):
                            tickers = fund_entry.get("tickers", [])
                            if tickers:
                                ticker = (
                                    tickers[0] if isinstance(tickers, list) else tickers
                                )
        ...
        if yf and test_ticker:
            try:
                ...
                entry["name"] = info.get("longName") or info.get(
                    "shortName", test_ticker
                )
                entry["issuer"] = info.get("fundFamily") or info.get(
                    "companyName", "Unknown"
                )
        ...
        # Only support US-registered funds via N-PORT
        # For non-US funds, use yfinance as fallback
        if ticker or isin:
    ...
    def ensure_holdings_available(
        ...
        # Check if holdings already exist before triggering ingestion
        # New structure: data/funds/[fund_name]/[date]/
        project_root = (
            self.base_path.parent.parent
            if self.base_path.name == "pipeline"
        ...
        if fund_dir.exists():
            # Check if there are any date directories with holdings
            date_dirs = [
                d
                for d in fund_dir.iterdir()
                if d.is_dir() and (d / "holdings.csv").exists()
            ]
            if date_dirs:
                # Holdings exist, no need to ingest
                print(f"Holdings already exist for {fund_name}, skipping ingestion")
                return True

        # Holdings don't exist, check if we should ingest
        domicile = entry.get("domicile", "")

        if domicile == "US" and entry.get("cik"):
            # Use N-PORT pipeline for US-registered funds
            try:
        ...
        # For non-US funds or cases without CIK, auto_snapshot will handle it via PrimaryHoldingsClient
        return True
